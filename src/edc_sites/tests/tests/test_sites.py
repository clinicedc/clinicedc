...
class TestSites(SiteTestCaseMixin, TestCase):
    ...
    def test_get_language_choices_for_site(self):
        ...
        language_choices = site_sites.get_language_choices_tuple(site)
        self.assertTupleEqual(language_choices, (("en", "English"), ("sw", "Swahili")))

        language_choices = site_sites.get_language_choices_tuple(site, other=True)
    ...
    def test_get_site_id_by_name(self):
        ...
        self.assertEqual(site_sites.get_by_attr("name", "mochudi").site_id, 10)

    def test_get_site_id_by_description(self):
        site_sites.initialize(initialize_site_model=True)
        site_sites.register(*self.default_sites)
        add_or_update_django_sites()
        self.assertEqual(site_sites.get_by_attr("description", "Mochudi").site_id, 10)

    def test_get_site_id_invalid(self):
        site_sites.initialize(initialize_site_model=True)
        site_sites.register(*self.default_sites)
        add_or_update_django_sites()
        self.assertRaises(SiteDoesNotExist, site_sites.get_by_attr, "name", "blahblah")
    ...
    @override_settings(EDC_SITES_REGISTER_DEFAULT=False)
    ...
    @override_settings(EDC_SITES_REGISTER_DEFAULT=False, EDC_SITES_UAT_DOMAIN=True)
    def test_uat_domain(self):
        site_sites.initialize(initialize_site_model=True)
        self.assertTrue(settings.EDC_SITES_UAT_DOMAIN)
        site_sites.register(*self.default_sites)
        add_or_update_django_sites()
        self.assertEqual(site_sites.get(10).domain, "mochudi.uat.bw.clinicedc.org")
        add_or_update_django_sites()
        obj = Site.objects.get(name="molepolole")
    ...
    @override_settings(EDC_SITES_REGISTER_DEFAULT=False, SITE_ID=SiteID(default=10))
    def test_country(self):
        ...
        self.assertEqual("botswana", site_sites.get_current_country())
        self.assertEqual(
            [s for s in self.default_sites if s.country == "botswana"],
            site_sites.get_by_country("botswana", aslist=True),
        )

    @override_settings(EDC_SITES_REGISTER_DEFAULT=False, EDC_SITES_UAT_DOMAIN=False)
    def test_register_sites(self):
        ...
        self.assertRaises(AlreadyRegistered, site_sites.register, site1)
        self.assertRaises(AlreadyRegistered, site_sites.register, site_bad1)
        self.assertRaises(AlreadyRegisteredName, site_sites.register, site_bad2)
        self.assertRaises(AlreadyRegisteredDomain, site_sites.register, site_bad3)
        ...
        self.assertEqual(site_sites.get_by_country("tanzania"), {1: site1})
        self.assertEqual(site_sites.get_by_country("uganda"), {2: site2})
    ...
    def test_register_default_site_domain2(self):
        ...
        for single_site in site_sites.all(aslist=True):
            self.assertRaises(AlreadyRegistered, site_sites.register, single_site)
        add_or_update_django_sites()
        site = Site.objects.get(id=1)
        self.assertEqual(Alias.objects.get(site=site).domain, "localhost")
    ...
    def test_alias_model(self):
        ...
        add_or_update_django_sites()
        self.assertEqual(
            [s.site_id for s in site_sites.all(aslist=True)], [10, 20, 30, 40, 50, 60]
        )
    ...
    def test_permissions_sites(self):
        ...
        self.assertFalse(site_sites.user_may_view_other_sites(request))
    ...
    def test_permissions_messages(self):
        ...
        self.assertRaises(
            InvalidSiteForUser,
            site_sites.user_may_view_other_sites,
        ...
        site_sites.user_may_view_other_sites(response.wsgi_request)
        self.assertIn(
