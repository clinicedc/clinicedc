...
def valid_subject_identifier_or_raise(
    ...
    if not re.match(trial_settings.subject_identifier_pattern, subject_identifier):
        if raise_exception:
            raise RegisteredSubjectError(
                f"Invalid subject identifier format. "
                f"Valid pattern is `{trial_settings.subject_identifier_pattern}`. "
                f"See `edc_protocol.trial_settings.subject_identifier_pattern`. "
