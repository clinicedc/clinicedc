...
class ReceiveHomeView(AuthsViewMixin, EdcViewMixin, NavbarViewMixin, EdcProtocolViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        receive_map = {
            r.order_id: r for r in Receive.objects.select_related("supplier").all()
        }
        for order in orders:
            receive = receive_map.get(order.pk)
            item_count = (
                ReceiveItem.objects.filter(order_item__order=order).count()
                if receive
                else 0
            )
            rows.append(
