...
from ..utils.process_return_request import dispatch_return
...
class ReturnRequestView(EdcViewMixin, NavbarViewMixin, EdcProtocolViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if return_request:
            ...
            remaining_count = max(0, (return_request.item_count or 0) - dispatched_count)
    ...
    def _get_return_request(self) -> ReturnRequest | None:
        ...
        if pk:
            try:
                ...
                messages.add_message(self.request, messages.ERROR, "Return request not found.")
        return None

    @staticmethod
    def delete_empty_return_request_and_redirect(request: WSGIRequest) -> HttpResponseRedirect:
        try:
    ...
    @staticmethod
    def create_return_request_and_redirect(request: WSGIRequest) -> HttpResponseRedirect:
    ...
    @staticmethod
    def update_item_count_expected_and_redirect(
        request: WSGIRequest,
    ...
    def post(self, request, *args, **kwargs):  # noqa: ARG002
        ...
        if not return_request and request.POST.get("delete_pk"):
            return self.delete_empty_return_request_and_redirect(request)

        # --- Phase 1b: create a new ReturnRequest and flag codes ---
        if not return_request:
            self.create_return_request_and_redirect(request)

        # --- Phase 2a: update the expected item count ---
        if return_request and request.POST.get("update_item_count"):
            self.update_item_count_expected_and_redirect(request, return_request)
        ...
        if stock_codes and return_request:
            try:
